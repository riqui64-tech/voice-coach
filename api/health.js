...
async function probe(token,model){
  ...
  try{
    const r=await fetch('https://ai-gateway.vercel.sh/v1/chat/completions',{
      ...
      body:JSON.stringify({model,stream:false,messages:[{role:'user',content:'Reply OK'}]})
    });
...
module.exports=async function handler(req,res){
  ...
  if(primary.ok)return res.status(200).json({ok:true,ready:true,degraded:false,token:true,gateway:true,activeModel:PRIMARY,primary,fallback:null,message:'Primary AI model ready'});
  const fallback=await probe(token,FALLBACK);
  if(fallback.ok)return res.status(200).json({ok:true,ready:true,degraded:true,token:true,gateway:true,activeModel:FALLBACK,primary,fallback,message:'Primary model unavailable; fallback AI model ready'});
